        ...
        if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) {
          ...
          if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch) {
            ...
            if (IFETCH_BUFFER.entry[ifetch_buffer_index].branch_taken != branch_prediction) {
              ...
              og_record_event(MISPRED_BRANCH, 1, cpu); // OG record event
    ...
    } else // not a cloudsuite trace
    {
      ...
      } else // successfully read the trace
      {
        ...
        if (!reads_sp && !reads_flags && writes_ip && !reads_other) {
          ...
          // conditional branch
          arch_instr.is_branch = 1;
          arch_instr.branch_taken = arch_instr.branch_taken; // don't change
                                                             // this
        ...
        if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) {
          ...
          if (IFETCH_BUFFER.entry[ifetch_buffer_index].is_branch) {
            ...
            l1i_prefetcher_branch_operate(IFETCH_BUFFER.entry[ifetch_buffer_index].ip, IFETCH_BUFFER.entry[ifetch_buffer_index].branch_type,
...
uint32_t O3_CPU::add_to_ifetch_buffer(ooo_model_instr *arch_instr) {
  ...
  {
    cout << (uint32_t)(arch_instr->source_reg isters[i]) << " ";
  }
      cout << endl;
      for (uint32_t i=0; i<MAX_INSTR_DESTINATIONS; i++)
  {
...
void O3_CPU::fetch_instruction() {
  ...
  for (uint32_t i = 0; i < IFETCH_BUFFER.SIZE; i++) {
    ...
    if (IFETCH_BUFFER.entry[index].translated == 0) {
      ...
      trace_packet.rob_position = -2;  // special position. RBERA_TODO: should
                                       // we give this highest priority?
      trace_packet.rob_part_type = -2; // special position. RBERA_TODO: should
                                       // we give this highest priority?
      ...
      if (rq_index != -2) {
        ...
        for (uint32_t j = 0; j < IFETCH_BUFFER.SIZE; j++) {
          if ((((IFETCH_BUFFER.entry[j].ip) >> LOG2_PAGE_SIZE) == ((IFETCH_BUFFER.entry[index].ip) >> LOG2_PAGE_SIZE)) &&
    ...
    if ((IFETCH_BUFFER.entry[index].translated == COMPLETED) && (IFETCH_BUFFER.entry[index].fetched == 0)) {
      ...
      fetch_packet.rob_position = -3;  // special position. RBERA_TODO: should
                                       // we give this highest priority?
      fetch_packet.rob_part_type = -3; // special position. RBERA_TODO: should
                                       // we give this highest priority?
...
void O3_CPU::decode_and_dispatch() {
  ...
  for (uint32_t i = 0; i < DECODE_BUFFER.SIZE; i++) {
    ...
    if (((!warmup_complete[cpu]) && (ROB.occupancy < ROB.SIZE)) ||
        ((DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle != 0) && (DECODE_BUFFER.entry[DECODE_BUFFER.head].event_cycle < current_core_cycle[cpu]) &&
...
void O3_CPU::do_scheduling(uint32_t rob_index) {
  ROB.entry[rob_index].reg_ready = 1; // reg_ready will be reset to 0 if there
                                      // is RAW dependency
...
void O3_CPU::reg_RAW_dependency(uint32_t prior, uint32_t current, uint32_t source_index) {
  for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
    ...
    if (ROB.entry[prior].destination_registers[i] == ROB.entry[current].source_registers[source_index]) {
      ...
      ROB.entry[prior].registers_instrs_depend_on_me.insert(current);              // this load cannot be executed until the prior store gets
                                                                                   // executed
      ROB.entry[prior].registers_index_depend_on_me[source_index].insert(current); // this load cannot be executed until the prior store gets
                                                                                   // executed
...
void O3_CPU::do_memory_scheduling(uint32_t rob_index) {
  ...
  if (not_available == 0) {
    ROB.entry[rob_index].scheduled = COMPLETED;
    if (ROB.entry[rob_index].executed == 0) { // it could be already set to
                                              // COMPLETED due to store-to-load
                                              // forwarding
      ROB.entry[rob_index].executed = INFLIGHT;
    }
...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index) {
  ...
  LQ.entry[lq_index].rob_position = compute_rob_position(rob_index, ROB.head);
  LQ.entry[lq_index].rob_part_type = rob_pos_get_part_type(LQ.entry[lq_index].rob_position);
  ...
  if (forwarding_index != SQ.SIZE) // we have a store-to-load forwarding
  {
    if ((SQ.entry[forwarding_index].fetched == COMPLETED) && (SQ.entry[forwarding_index].event_cycle <= current_core_cycle[cpu])) {
      LQ.entry[lq_index].physical_address = (SQ.entry[forwarding_index].physical_address & ~(uint64_t)((1 << LOG2_BLOCK_SIZE) - 1)) |
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << SQ.entry[forwarding_index].instr_id << " remain_num_ops: " << ROB.entry[fwr_rob_index].num_mem_ops << " cycle: " << current_core_cycle[cpu]
             << endl;
      });
      ...
      ; // store is not executed yet, forwarding will be handled by
        // execute_store()
    // loads, that are eventually be forwarded from SQ but the stores are not
    // completed yet, are still in LQ which means they can still generate memory
    // request to cache hierarchy
  }

  // succesfully added to the load queue
  ROB.entry[rob_index].source_added[data_index] = 1;

  if (LQ.entry[lq_index].virtual_address && (LQ.entry[lq_index].producer_id == UINT64_MAX)) { // not released and no
                                                                                              // forwarding
  ...
  DP(if (warmup_complete[cpu]) {
    ...
    cout << " fetched: " << +LQ.entry[lq_index].fetched << " index: " << lq_index << " occupancy: " << LQ.occupancy << " cycle: " << current_core_cycle[cpu]
         << endl;
  });
}
...
void O3_CPU::mem_RAW_dependency(uint32_t prior, uint32_t current, uint32_t data_index, uint32_t lq_index) {
  for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
    ...
    if (ROB.entry[prior].destination_memory[i] == ROB.entry[current].source_memory[data_index]) { //  store-to-load
                                                                                                  //  forwarding check

      // we need to mark this dependency in the ROB since the producer might not
      // be added in the store queue yet
      ROB.entry[prior].memory_instrs_depend_on_me.insert(current); // this load cannot be executed until the prior store gets
                                                                   // executed
      ...
      DP(if (warmup_complete[cpu]) {
        cout << "[LQ] " << __func__ << " RAW producer instr_id: " << ROB.entry[prior].instr_id << " consumer_id: " << ROB.entry[current].instr_id
             << " lq_index: " << lq_index;
...
void O3_CPU::add_store_queue(uint32_t rob_index, uint32_t data_index) {
  ...
  */
  ...
  SQ.entry[sq_index].rob_position = compute_rob_position(rob_index, ROB.head);
  SQ.entry[sq_index].rob_part_type = rob_pos_get_part_type(SQ.entry[sq_index].rob_position);
  ...
  while (store_issued < SQ_WIDTH) {
    if (RTS0[RTS0_head] < SQ_SIZE) {
      ...
      if (SQ.entry[sq_index].event_cycle <= current_core_cycle[cpu]) {
        ...
        DP(if (warmup_complete[cpu]) {
          cout << "[RTS0] " << __func__ << " instr_id: " << SQ.entry[sq_index].instr_id << " rob_index: " << SQ.entry[sq_index].rob_index
  ...
  while (load_issued < LQ_WIDTH) {
    if (RTL0[RTL0_head] < LQ_SIZE) {
      ...
      if (LQ.entry[lq_index].event_cycle <= current_core_cycle[cpu]) {
        ...
        DP(if (warmup_complete[cpu]) {
          cout << "[RTL0] " << __func__ << " instr_id: " << LQ.entry[lq_index].instr_id << " rob_index: " << LQ.entry[lq_index].rob_index
               << " is popped to RTL0";
          cout << " head: " << RTL0_head << " tail: " << RTL0_tail << endl;
        });
        ...
        load_issued++;
        og_record_event(LOAD_INST_ISSUE, 1, cpu);
      }
    } else {
      // DP (if (warmup_complete[cpu]) {
    ...
    ITERATE_SET(dependent, ROB.entry[rob_index].memory_instrs_depend_on_me, ROB_SIZE) {
      ...
      for (uint32_t j = 0; j < NUM_INSTR_SOURCES; j++) { // which one is dependent?
        if (ROB.entry[dependent].source_memory[j] && ROB.entry[dependent].source_added[j]) {
          if (ROB.entry[dependent].source_memory[j] == SQ.entry[sq_index].virtual_address) { // this is required since a
            ...
            LQ.entry[lq_index].physical_address = (SQ.entry[sq_index].physical_address & ~(uint64_t)((1 << LOG2_BLOCK_SIZE) - 1)) |
                                                  (LQ.entry[lq_index].virtual_address & ((1 << LOG2_BLOCK_SIZE) - 1));
            LQ.entry[lq_index].translated = COMPLETED;
            LQ.entry[lq_index].fetched = COMPLETED;
            ...
            DP(if (warmup_complete[cpu]) {
              ...
              cout << " full_addr: " << LQ.entry[lq_index].physical_address << dec << " is forwarded by store instr_id: ";
              cout << SQ.entry[sq_index].instr_id << " remain_num_ops: " << ROB.entry[fwr_rob_index].num_mem_ops << " cycle: " << current_core_cycle[cpu]
                   << endl;
            });
...
void O3_CPU::complete_execution(uint32_t rob_index) {
  ...
  {
    if (ROB.entry[rob_index].num_mem_ops == 0) // all source memory operands are
                                               // ready
    {
      if ((ROB.entry[rob_index].executed == INFLIGHT) && (ROB.entry[rob_index].event_cycle <= current_core_cycle[cpu])) {
        ROB.entry[rob_index].executed = COMPLETED;
    ...
    for (uint32_t j = 0; j < NUM_INSTR_SOURCES; j++) {
      if (ROB.entry[rob_index].registers_index_depend_on_me[j].search(i)) {
        ...
        DP(if (warmup_complete[cpu]) {
          ...
          cout << ROB.entry[i].instr_id << " reg_index: " << +ROB.entry[i].source_registers[j] << " num_reg_dependent: " << ROB.entry[i].num_reg_dependent
  ...
  if (is_it_tlb) {
    ...
    for (uint32_t j = 0; j < IFETCH_BUFFER.SIZE; j++) {
      if (((IFETCH_BUFFER.entry[j].ip) >> LOG2_PAGE_SIZE) == ((complete_ip) >> LOG2_PAGE_SIZE)) {
        IFETCH_BUFFER.entry[j].translated = COMPLETED;
        // we did not fetch this instruction's cache line, but we did translate
        // it
        IFETCH_BUFFER.entry[j].fetched = 0;
    ...
    // remove this entry
    queue->remove_queue(&queue->entry[index], current_core_cycle[queue->entry[index].cpu]);
  } else {
...
void O3_CPU::complete_data_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
  uint32_t index = queue->head, rob_index = queue->entry[index].rob_index, sq_index = queue->entry[index].sq_index, lq_index = queue->entry[index].lq_index;
  ...
  if (is_it_tlb) // DTLB
  {
    if (queue->entry[index].type == RFO) {
      SQ.entry[sq_index].physical_address =
          (queue->entry[index].data_pa << LOG2_PAGE_SIZE) | (SQ.entry[sq_index].virtual_address & ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " DTLB_FETCH_DONE translation: " << +SQ.entry[sq_index].translated << hex
             << " page: " << (SQ.entry[sq_index].physical_address >> LOG2_PAGE_SIZE);
      ...
      LQ.entry[lq_index].physical_address =
          (queue->entry[index].data_pa << LOG2_PAGE_SIZE) | (LQ.entry[lq_index].virtual_address & ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
      LQ.entry[lq_index].translated = COMPLETED;
      LQ.entry[lq_index].event_cycle = current_core_cycle[cpu];
      ...
      // RBERA: translation has been done for this load request.
      ...
      if (LQ.entry[lq_index].went_offchip_pred && knob::enable_ddrp) {
        if (!knob::og_enable || oogway->is_ocp_enabled()) {
          issue_ddrp_request(lq_index, 0);
        } else if (knob::og_enable) {
          oogway->incr_ocp_throttled();
        }
      }
      ...
      DP(if (warmup_complete[cpu]) {
        ...
        cout << " DTLB_FETCH_DONE translation: " << +LQ.entry[lq_index].translated << hex
             << " page: " << (LQ.entry[lq_index].physical_address >> LOG2_PAGE_SIZE);
  ...
  } else // L1D
  {
    if (queue->entry[index].type == RFO) {
      ...
      measure_pipeline_bubble_stats(lq_index, rob_index);

      // monitor the PCs of dorsal/frontal loads
      monitor_loads(lq_index);
  ...
  if (provider->store_merged) {
    ITERATE_SET(merged, provider->sq_index_depend_on_me, SQ.SIZE) {
      ...
      SQ.entry[merged].physical_address =
          (provider->data_pa << LOG2_PAGE_SIZE) | (SQ.entry[merged].virtual_address & ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
  ...
  if (provider->load_merged) {
    ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
      ...
      LQ.entry[merged].physical_address =
          (provider->data_pa << LOG2_PAGE_SIZE) | (LQ.entry[merged].virtual_address & ((1 << LOG2_PAGE_SIZE) - 1)); // translated address
      ...
      // RBERA: translation has been done via merged translation
      // Thus we have the physical address. So now we can issue
      // a DDRP request, if this LQ entry is predicted to go offchip.
...
void O3_CPU::handle_merged_load(PACKET *provider) {
  ITERATE_SET(merged, provider->lq_index_depend_on_me, LQ.SIZE) {
    ...
    if (knob::offchip_pred_mark_merged_load) {
      LQ.entry[merged].went_offchip = LQ.entry[provider->lq_index].went_offchip; // if provider went offchip,
                                                                                 // mark merged LQ entry
                                                                                 // offchip
    }
...
void O3_CPU::retire_rob() {
  for (uint32_t n = 0; n < RETIRE_WIDTH; n++) {
    ...
    if (knob::enable_ddrp_monitor && ddrp_monitor) {
      ddrp_monitor->monitor_instr(current_core_cycle[cpu]);
    }

    // call Oogway hook
    if (oogway) {
      oogway->incr_inst();
      if (knob::og_epoch_by_inst && oogway->check_inst_epoch_end()) {
        og_state_t *sys_state = og_capture_state();
        oogway->train_and_take_action(sys_state);
        oogway->incr_nr_epochs();
        og_next_epoch();
      }
    }
  }
...
uint32_t O3_CPU::count_dependency(uint32_t rob_index, std::vector<uint32_t> &cat_dependents) {
  std::bitset<ROB_SIZE> dependent_rob_indices;
  // cout << "[BEGIN tracking dependency for ROB " << rob_index << "]" << endl;
  count_dependency_recursive(rob_index, dependent_rob_indices, cat_dependents, 0);
  // cout << "[END tracking dependency for ROB " << rob_index << "]" << endl;

  return dependent_rob_indices.count();
}

void O3_CPU::count_dependency_recursive(uint32_t rob_index, std::bitset<ROB_SIZE> &dependent_rob_indices, std::vector<uint32_t> &cat_dependents,
                                        uint32_t level) {
  if (knob::load_hit_dependency_max_level && level == knob::load_hit_dependency_max_level) {
    return; // bound the recusion depth
  }

  TYPE dependents[ROB_SIZE + 1];
  uint32_t count = ROB.entry[rob_index].registers_instrs_depend_on_me.expand(dependents, ROB_SIZE);
  if (count == 0) {
    return;
  }

  ITERATE_SET(i, ROB.entry[rob_index].registers_instrs_depend_on_me, ROB_SIZE) {
    if (!dependent_rob_indices.test(i)) {
      // cout << "[depth " << std::setw(4) << level << "]"
      //     << "ROB " << std::setw(3) << rob_index << " depends on " << "<"
      //     << std::setw(3) << i << ","
      //     << std::setw(8) << hex << ROB.entry[i].instr_id << dec << ","
      //     << std::setw(8) << hex << ROB.entry[i].ip << dec << ","
      //     << ">" << endl;
      dependent_rob_indices.set(i);
      if (ROB.entry[i].is_branch) {
        if (ROB.entry[i].branch_mispredicted) {
          cat_dependents[DEP_INSTR_BRANCH_MISPRED]++;
        } else {
          cat_dependents[DEP_INSTR_BRANCH_CORRECT]++;
        }
      }
      if (ROB.entry[i].is_load) {
        cat_dependents[DEP_INSTR_LOAD]++;
      }
      count_dependency_recursive(i, dependent_rob_indices, cat_dependents, level + 1);
    }
  }
}

void O3_CPU::debug_dependents(uint32_t rob_index) {
  MYLOG("-------------------------------------------");
  MYLOG("[current entry] %s", ROB.entry[rob_index].to_string().c_str());
  MYLOG("ROB_DUMP\n%s", rob_to_string().c_str());
  MYLOG("-------------------------------------------");
}

string O3_CPU::rob_to_string() {
  stringstream ss;
  if (ROB.head < ROB.tail) {
    for (uint32_t i = ROB.head; i < ROB.tail; i++) {
      ss << "[ROB " << std::setw(3) << i << "] ";
      ss << ROB.entry[i].to_string() << endl;
    }
  } else {
    for (uint32_t i = ROB.head; i < ROB.SIZE; i++) {
      ss << "[ROB " << std::setw(3) << i << "] ";
...
void O3_CPU::measure_pipeline_bubble_stats(uint32_t lq_index, uint32_t rob_index) {
  ...
  if (ROB.entry[rob_index].rob_head_cycle == 0) {
    // this load has NOT blocked the ROB
    stats.bubble.rob_non_head++;
  } else {
    // this load has blocked the ROB
    assert(rob_index == ROB.head);
    stats.bubble.rob_head++;
  }

  if (LQ.entry[lq_index].went_offchip) {
    stats.bubble.went_offchip++;
    if (ROB.entry[rob_index].rob_head_cycle != 0) {
      stats.bubble.went_offchip_rob_head++;
    } else {
    ...
    assert(rob_part_id < knob::num_rob_partitions);

    // max bubbles
    if (bubbles > bubble_max[rob_part_id]) {
      bubble_max[rob_part_id] = bubbles;
    }
    // min bubbles
    if (bubbles < bubble_min[rob_part_id]) {
      bubble_min[rob_part_id] = bubbles;
    }
    // total bubbles
    bubble_tot[rob_part_id] += bubbles;
    // count
    bubble_cnt[rob_part_id]++;

    // trace
    if (knob::enable_offchip_tracing && warmup_complete[cpu]) {
      tracer.record(bubbles);
    }
  }
}

void O3_CPU::monitor_loads(uint32_t lq_index) {
  uint64_t load_ip = LQ.entry[lq_index].ip;
  uint8_t went_offchip = LQ.entry[lq_index].went_offchip;
  uint32_t rob_part_id = get_rob_parition_id(LQ.entry[lq_index].rob_position);
  assert(rob_part_id < knob::num_rob_partitions);
  uint8_t rob_part_type = rob_part_id_get_part_type(rob_part_id);
  assert(rob_part_type < NUM_PARTITION_TYPES);

  // stats per ROB parition
  load_per_rob_part_stats[rob_part_type].total_loads++;
  if (went_offchip) {
    load_per_rob_part_stats[rob_part_type].loads_went_offchip++;
  }
  ...
  if (it != load_per_ip_stats.end()) {
    ...
    if (went_offchip) {
      ...
      it->second.loads_went_offchip_pos_hist[rob_part_id]++;
    }
  } else {
...
int8_t O3_CPU::rob_part_id_get_part_type(uint32_t rob_part_id) {
  if (rob_part_id_is_frontal(rob_part_id)) {
    return FRONTAL;
  } else if (rob_part_id_is_dorsal(rob_part_id)) {
    return DORSAL;
  } else {
    return NONE;
  }
...
uint32_t O3_CPU::get_source_index_from_rob(uint32_t rob_index, uint32_t lq_index) {
  uint32_t source_index = NUM_INSTR_SOURCES, num_sources = 0;
  for (uint32_t index = 0; index < NUM_INSTR_SOURCES; ++index) {
    if (ooo_cpu[cpu].ROB.entry[rob_index].lq_index[index] == lq_index) {
      source_index = index;
      num_sources++;
    }
  }
  assert(source_index != NUM_INSTR_SOURCES);
  assert(num_sources == 1); // is this true? Can one macro x86 instruction
                            // generate more than one memory request, both
                            // occupying same LQ entry?
  return source_index;
}
...
void O3_CPU::issue_ddrp_request(uint32_t lq_index, uint32_t call_type) {
  ...
  if (dram_controller->get_occupancy(1, LQ.entry[lq_index].physical_address >> LOG2_BLOCK_SIZE) ==
      dram_controller->get_size(1, LQ.entry[lq_index].physical_address >> LOG2_BLOCK_SIZE)) // check RQ's occupancy
  {
...
//---------------------------------------//
// OOGWAY
// Oogway dynamically orchestrates
// prefetchers and off-chip predictor.
//---------------------------------------//

void O3_CPU::og_init() {
  if (knob::og_enable) {
    oogway = new Oogway(cpu);
  }
}

void O3_CPU::og_print_config() {
  if (oogway) {
    oogway->print_config();
  }
}

void O3_CPU::og_record_event(og_sysevent_type_t sevent, uint64_t val, uint32_t cpu) {
  if (oogway) {
    oogway->record_sys_event(sevent, val, cpu);
  }
}

og_state_t *O3_CPU::og_capture_state() {
  og_state_t *sys_state = new og_state_t();
  assert(sys_state);

  // Aux information - not used as a state feature
  sys_state->epoch_num = oogway->get_epoch_num();
  sys_state->num_retired_insts = num_retired;

  // accuracy from prefetchers based on coordination mode
  if (knob::og_multi_prefetcher_enable) {
    if (knob::og_coordination_mode == "l1d_l2c") {
      // L1D + L2C coordination mode
      assert(L1D.l1d_prefetchers.size() == 1);
      assert(L2C.prefetchers.size() == 1);
      sys_state->pref_acc[0] = L1D.l1d_prefetchers[0]->get_accuracy();
      sys_state->pref_acc[1] = L2C.prefetchers[0]->get_accuracy();
    } else if (knob::og_coordination_mode == "l2c_l2c") {
      // L2C + L2C coordination mode
      assert(L2C.prefetchers.size() == 2);
      sys_state->pref_acc[0] = L2C.prefetchers[0]->get_accuracy();
      sys_state->pref_acc[1] = L2C.prefetchers[1]->get_accuracy();
    }
  } else {
    if (knob::og_prefetcher_level == 2) {
      assert(L2C.prefetchers.size() == 1);
      sys_state->pref_acc[0] = L2C.prefetchers[0]->get_accuracy();
    } else if (knob::og_prefetcher_level == 1) {
      assert(L1D.l1d_prefetchers.size() == 1);
      sys_state->pref_acc[0] = L1D.l1d_prefetchers[0]->get_accuracy();
    }
  }
  ...
  // accuracy from OCP
  sys_state->ocp_acc = offchip_pred ? offchip_pred->get_accuracy() : 100.0;

  // ZLANG: Bandwidth related features
  sys_state->overall_bw = dram_controller->get_bw(cpu);
  sys_state->pref_bw = dram_controller->get_pf_bw(cpu);
  sys_state->ocp_bw = dram_controller->get_ocp_bw(cpu);
  sys_state->bw_needed = dram_controller->get_bw_needed(cpu);

  sys_state->total_scheduled = dram_controller->get_total_scheduled(cpu);
  sys_state->pf_scheduled = dram_controller->get_pf_scheduled(cpu);
  sys_state->ocp_scheduled = dram_controller->get_ocp_scheduled(cpu);
  sys_state->demand_scheduled = dram_controller->get_demand_scheduled(cpu);

  // cout << "overall_bw: " << sys_state->overall_bw << endl
  // << "pref_bw: " << sys_state->pref_bw << endl
  // << "ocp_bw: " << sys_state->ocp_bw << endl
  // << "demand_bw: " << dram_controller->get_demand_bw(cpu) << endl
  // << "bw_needed: " << sys_state->bw_needed << endl;

  // ZLANG: pollution
  sys_state->pref_pollution = uncore.LLC.get_prefetch_pollution_count();

  return sys_state;
}

void O3_CPU::og_next_epoch() {
  // reset prefetch accuracy based on coordination mode
  if (knob::og_multi_prefetcher_enable) {
    if (knob::og_coordination_mode == "l1d_l2c") {
      // L1D + L2C coordination mode
      assert(L1D.l1d_prefetchers.size() == 1);
      assert(L2C.prefetchers.size() == 1);
      L1D.l1d_prefetchers[0]->reset_accuracy();
      L2C.prefetchers[0]->reset_accuracy();
    } else if (knob::og_coordination_mode == "l2c_l2c") {
      // L2C + L2C coordination mode
      assert(L2C.prefetchers.size() == 2);
      L2C.prefetchers[0]->reset_accuracy();
      L2C.prefetchers[1]->reset_accuracy();
    }
  } else {
    if (knob::og_prefetcher_level == 1) {
      assert(L1D.l1d_prefetchers.size() == 1);
      L1D.l1d_prefetchers[0]->reset_accuracy();
    } else if (knob::og_prefetcher_level == 2) {
      assert(L2C.prefetchers.size() == 1);
      L2C.prefetchers[0]->reset_accuracy();
    }
  }
  ...
  // reset OCP accuracy
  if (offchip_pred) {
    offchip_pred->reset_accuracy();
  }

  uncore.LLC.reset_prefetch_pollution_count();

  dram_controller->reset_og_scheduled(cpu);
}