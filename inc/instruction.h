...
public:
  ...
  uint8_t destination_registers[NUM_INSTR_DESTINATIONS_SPARC]; // output
                                                               // registers
  uint8_t source_registers[NUM_INSTR_SOURCES];                 // input registers
...
public:
  uint64_t instr_id, ip, fetch_producer, producer_id, translated_cycle, fetched_cycle, execute_begin_cycle, retired_cycle, event_cycle,
      rob_head_cycle; // when the instruction became ROB head

  uint8_t is_branch, is_memory, is_load, is_store, branch_taken, branch_mispredicted, branch_prediction_made, translated, data_translated,
      source_added[NUM_INSTR_SOURCES], destination_added[NUM_INSTR_DESTINATIONS_SPARC], is_producer, is_consumer, reg_RAW_producer, reg_ready, mem_ready,
      asid[2], reg_RAW_checked[NUM_INSTR_SOURCES];
  ...
  uint32_t lq_index[NUM_INSTR_SOURCES], sq_index[NUM_INSTR_DESTINATIONS_SPARC], forwarding_index[NUM_INSTR_DESTINATIONS_SPARC];

  uint8_t mem_source_went_offchip[NUM_INSTR_SOURCES]; // RBERA: to track which
                                                      // memory sources went
                                                      // offchip
  ...
  string to_string() {
    ...
    ss << "|" << hex << std::setw(8) << instr_id << dec << "|" << hex << std::setw(8) << (uint64_t)ip << dec << "|" << "<" << (uint32_t)is_branch << "-"
       << (uint32_t)branch_mispredicted << "-" << (uint32_t)branch_taken << ">" << "<" << (uint32_t)is_producer << "-" << (uint32_t)is_consumer << "-"
       << (uint32_t)reg_RAW_producer << ">" << "<" << (uint64_t)is_memory << "-" << (uint32_t)is_load << "-" << (uint32_t)is_store << ">" << "|";
