...
public:
  ...
  /**
   * @return False if the tag wasn't found and true if the pattern bit was
   * successfully set
   */
  ...
  /* NOTE: `region_number` is probably truncated since it comes from the filter
   * table */
...
public:
  ...
  /**
   * First searches for a PC+Address match. If no match is found, returns all
   * PC+Offset matches.
   * @return All un-rotated patterns if matches were found, returns an empty
   * vector otherwise
   */
...
private:
  ...
  uint64_t build_key(uint64_t pc, uint64_t address) {
     ...
     * The index must be computed from only PC+Offset to ensure that all entries
     * with the same PC+Offset end up in the same set */
...
public:
  PrefetchStreamer(PTracker *_tracker, int size, int pattern_len, int debug_level = 0, int num_ways = 16)
      : m_tracker(_tracker), Super(size, num_ways, debug_level), pattern_len(pattern_len) {
    if (this->debug_level >= 1) {
  ...
  int prefetch(uint32_t id, CACHE *cache, uint64_t block_address) {
    if (this->debug_level >= 2) {
    ...
    pattern[region_offset] = 0; /* accessed block will be automatically fetched
                                   if necessary (miss) */
    int pf_offset;
      ...
      for (int sgn = +1; sgn >= -1; sgn -= 2) {
        ...
        if (0 <= pf_offset && pf_offset < this->pattern_len && pattern[pf_offset] > 0) {
          ...
          if (cache->PQ.occupancy + cache->MSHR.occupancy < cache->MSHR.SIZE - 1 && cache->PQ.occupancy < cache->PQ.SIZE) {
            if (!knob::og_enable || ooo_cpu[cache->cpu].oogway->is_prefetch_enabled(id)) {
              cache->prefetch_line(0, base_addr, pf_address, pattern[pf_offset], 0);
              pf_issued += 1;
              pattern[pf_offset] = 0;
              if (m_tracker) {
                m_tracker->track_pref(pf_address);
              }
            } else if (knob::og_enable) {
              ooo_cpu[cache->cpu].oogway->incr_pf_throttled();
            }
          } else {
...
private:
  ...
  uint64_t build_key(uint64_t region_number) {
    return hash_index(region_number, this->index_len);
  }
...
public:
  ...
  /*========== stats ==========*/
  /* NOTE: the BINGO code submitted for DPC3 (this code) does not call any of
   * these methods. */
...
private:
   ...
   * @return The appropriate prefetch level for all blocks based on PHT output
   * or an empty vector if no blocks should be prefetched
   */
  ...
  /**
   * Uses a voting mechanism to produce a prefetching pattern from a set of
   * footprints.
   * @param x The patterns obtained from all PC+Offset matches
   * @return  The appropriate prefetch level for all blocks based on BINGO's
   * voting thresholds or an empty vector if no blocks should be prefetched
   */
