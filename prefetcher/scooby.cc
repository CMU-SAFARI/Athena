...
void Scooby::init_knobs() {
  ...
  if (knob::scooby_access_debug) {
    cout << "***WARNING*** setting knob::scooby_max_pcs, "
            "knob::scooby_max_offsets, and knob::scooby_max_deltas to large "
            "value as knob::scooby_access_debug is true"
         << endl;
...
Scooby::Scooby(string type, CACHE *cache) : Prefetcher(type), parent(cache) {
  init_knobs();
  init_stats();
...
void Scooby::print_config() {
  cout << "scooby_alpha " << knob::scooby_alpha << endl
       ...
       << "scooby_enable_direct_pref_issue " << knob::scooby_enable_direct_pref_issue << endl
       << "scooby_pref_at_lower_level " << knob::scooby_pref_at_lower_level << endl
       << endl
...
void Scooby::invoke_prefetcher(uint64_t pc, uint64_t address, uint8_t cache_hit, uint8_t type, vector<uint64_t> &pref_addr) {
  ...
  /* Measure state.
   * state can contain per page local information like delta signature, pc
   * signature etc. it can also contain global signatures like last three branch
   * PCs etc.
   */
  ...
  /* issue prefetches */
  if (knob::scooby_enable_direct_pref_issue) {
    for (uint32_t addr_index = 0; addr_index < pref_addr.size(); ++addr_index) {
      if (!knob::og_enable || ooo_cpu[parent->cpu].oogway->is_prefetch_enabled(id)) {
        parent->prefetch_line(pc, address, pref_addr[addr_index], knob::scooby_pref_at_lower_level ? FILL_LLC : FILL_L2, 0);
        if (ptracker) {
          ptracker->track_pref(pref_addr[addr_index]);
        }
      } else if (knob::og_enable) {
        ooo_cpu[parent->cpu].oogway->incr_pf_throttled();
      }
    }
    pref_addr.clear();
  }
    ...
    if (st_index != signature_table.end()) {
      ...
      if (found) {
        for (uint32_t index = 0; index < conf_thresholds.size(); ++index) {
          /* scooby_last_pref_offset_conf_thresholds is a sorted list in
...
void Scooby::update_stats(State *state, uint32_t action_index, uint32_t degree) {
  ...
  if (it != state_action_dist2.end()) {
    it->second[action_index]++;
    it->second[knob::scooby_max_actions]++; /* counts total occurences of this
                                               state */
  } else {
    vector<uint64_t> act_dist;
    act_dist.resize(knob::scooby_max_actions + 1, 0);
    act_dist[action_index]++;
    act_dist[knob::scooby_max_actions]++; /* counts total occurences of this
                                             state */
...
void Scooby::dump_stats() {
  cout << "scooby_st_lookup " << stats.st.lookup << endl
       ...
       << endl
       // << "scooby_predict_shaggy_called " << stats.predict.shaggy_called <<
       // endl
