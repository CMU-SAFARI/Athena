...
void CACHE::l2c_prefetcher_initialize() {
  ...
  for (uint32_t index = 0; index < prefetchers.size(); ++index) {
    // Set ID based on coordination mode
    if (knob::og_multi_prefetcher_enable && knob::og_coordination_mode == "l1d_l2c") {
      // In L1D+L2C mode, L2C prefetchers start from ID 1 (L1D gets ID 0)
      prefetchers[index]->set_id(index + 1);
    } else {
      // In L2C+L2C mode or legacy mode, L2C prefetchers start from ID 0
      prefetchers[index]->set_id(index);
    }
    cout << " L2C pref " << prefetchers[index]->get_type() << " got ID " << prefetchers[index]->get_id() << endl;
  }
  ...
  if (knob::ptracker_enable) {
    for (uint32_t i = 0; i < prefetchers.size(); ++i) {
      prefetchers[i]->create_ptracker(knob::ptracker_num_sets, knob::ptracker_num_ways);
    }
  }
...
uint32_t CACHE::l2c_prefetcher_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type, uint32_t metadata_in) {
  ...
  pref_addr.clear();
  for (uint32_t index = 0; index < prefetchers.size(); ++index) {
    if (knob::l2c_prefetcher_types[index].compare("ipcp")) { /* means not IPCP */
      /* record the demand */
      if (prefetchers[index]->ptracker) {
        prefetchers[index]->ptracker->track_demand(addr);
      }
    ...
    if (knob::l2c_prefetcher_types[index].compare("spp_dev2") && knob::l2c_prefetcher_types[index].compare("spp_ppf_dev") &&
        knob::l2c_prefetcher_types[index].compare("mlop") && knob::l2c_prefetcher_types[index].compare("bingo") &&
        knob::l2c_prefetcher_types[index].compare("ipcp") && !pref_addr.empty()) {
      for (uint32_t addr_index = 0; addr_index < pref_addr.size(); ++addr_index) {
        if (!ooo_cpu[cpu].oogway || ooo_cpu[cpu].oogway->is_prefetch_enabled(prefetchers[index]->get_id())) {
          prefetch_line(ip, addr, pref_addr[addr_index], knob::l2c_prefetcher_force_prefetch_at_llc ? FILL_LLC : FILL_L2, 0);
          if (prefetchers[index]->ptracker) {
            prefetchers[index]->ptracker->track_pref(pref_addr[addr_index]);
          }
        } else if (ooo_cpu[cpu].oogway) {
