...
void SPP_dev2::invoke_prefetcher(uint64_t ip, uint64_t addr, uint8_t cache_hit, uint8_t type, std::vector<uint64_t> &pref_addr) {
  ...
  uint32_t page_offset = (addr >> LOG2_BLOCK_SIZE) & (PAGE_SIZE / BLOCK_SIZE - 1), last_sig = 0, curr_sig = 0, confidence_q[L2C_MSHR_SIZE], depth = 0;

  int32_t delta = 0, delta_q[L2C_MSHR_SIZE];
  ...
  do {
    ...
    for (uint32_t i = pf_q_head; i < pf_q_tail; i++) {
      if (confidence_q[i] >= knob::spp_dev2_pf_threshold) {
        ...
        if ((addr & ~(PAGE_SIZE - 1)) == (pf_addr & ~(PAGE_SIZE - 1))) { // Prefetch request is in the same
                                                                         // physical page
          if (FILTER.check(pf_addr, confidence_q[i] >= knob::spp_dev2_fill_threshold ? SPP_L2C_PREFETCH : SPP_LLC_PREFETCH, GHR)) {
            if (!ooo_cpu[m_parent_cache->cpu].oogway || ooo_cpu[m_parent_cache->cpu].oogway->is_prefetch_enabled(id)) {
              m_parent_cache->prefetch_line(ip, addr, pf_addr, ((confidence_q[i] >= knob::spp_dev2_fill_threshold) ? FILL_L2 : FILL_LLC),
                                            0); // Use addr (not base_addr) to obey the same physical page
                                                // boundary
            } else if (knob::og_enable) {
              ooo_cpu[m_parent_cache->cpu].oogway->incr_pf_throttled();
            }
            stats.pref.total++;
            ...
            SPP_DP(cout << "[ChampSim] " << __func__ << " base_addr: " << hex << base_addr << " pf_addr: " << pf_addr;
                   ...
                   cout << " depth: " << i << " FILL_THRESH: " << knob::spp_dev2_fill_threshold
                        << " fill_level: " << ((confidence_q[i] >= knob::spp_dev2_fill_threshold) ? FILL_L2 : FILL_LLC) << endl;);
          }
    ...
    if (lookahead_way < PT_WAY) {
      uint32_t set = spp::get_hash(curr_sig) % PT_SET;
    ...
    if (breadth >= stats.breadth.max) {
      stats.breadth.max = breadth;
    }
    if (breadth <= stats.breadth.min) {
      stats.breadth.min = breadth;
    }
#ifdef LOOKAHEAD_ON
  } while (do_lookahead);
#endif
  ...
  if (depth >= stats.depth.max) {
    stats.depth.max = depth;
  }
  if (depth <= stats.depth.min) {
    stats.depth.min = depth;
  }
}
...
void SPP_dev2::dump_stats() {
  ...
  sort(pairs2.begin(), pairs2.end(),
       [](pair<uint32_t, unordered_map<int32_t, uint64_t>> &a, pair<uint32_t, unordered_map<int32_t, uint64_t>> &b) { return a.first < b.first; });
