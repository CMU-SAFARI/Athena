...
/**
 * Updates MLOP's state based on the most recent trigger access (LOAD
 * miss/prefetch-hit).
...
void MLOP::access(uint64_t block_number) {
  ...
  if (this->debug_level >= 2) {
    cout << "[MLOP::access] zone_number=0x" << hex << zone_number << dec << ", zone_offset=" << zone_offset << endl;
  }
...
void MLOP::prefetch(CACHE *cache, uint64_t block_number) {
  if (this->debug_level >= 2) {
    cout << "MLOP::prefetch(cache=" << cache->NAME << "-" << cache->cpu << ", block_number=0x" << hex << block_number << dec << ")" << endl;
  }
  int pf_issued = 0;
  uint64_t zone_number = block_number / this->blocks_in_zone;
  int zone_offset = block_number % this->blocks_in_zone;
  ...
  for (uint32_t d = 0; d < PF_DEGREE; d += 1) {
    for (auto &cur_pf_offset : this->pf_offset[d]) {
      ...
      if (access_map[offset_to_prefetch] == MLOP_State::PREFTCH && prefetch_map[offset_to_prefetch] <= this->pf_level[d]) {
        continue;
      }
      ...
      if (this->is_inside_zone(offset_to_prefetch) && cache->PQ.occupancy < cache->PQ.SIZE &&
        ...
        if (!ooo_cpu[cache->cpu].oogway || ooo_cpu[cache->cpu].oogway->is_prefetch_enabled(id)) {
          cache->prefetch_line(0, base_addr, pf_addr, this->pf_level[d], 0);
          // assert(ok == 1);
          this->mark(pf_block_number, MLOP_State::PREFTCH, this->pf_level[d]);
          pf_issued += 1;
          if (ptracker) {
            ptracker->track_pref(pf_addr);
          }
        } else if (ooo_cpu[cache->cpu].oogway) {
